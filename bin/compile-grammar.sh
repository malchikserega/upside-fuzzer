...
OUT_DIR=""
# This script lives in bin/ (one directory below the actual repo root) --
# climb one extra level versus a script at the root itself.
...
First-party pipeline (RESTler retired — no Docker, no external compiler):
  1. Parses the OpenAPI/Swagger spec directly (tools/grammar/grammarc/oas.py)
  2. If --src is given, runs tools/dotnet/analyzer/ (a real Microsoft.CodeAnalysis.CSharp
...
# `cd "$ROOT_DIR/tools/grammar"` below (needed so `python3 -m grammarc.cli` can
# find its own package). Without this, a relative --out/--swagger/--dict/--src resolves
...
if [ -n "$SOURCE_CODE_PATH" ]; then
    ...
    BUILD_LOG="$(mktemp)"
    if ! ( cd "$ROOT_DIR/tools/dotnet/analyzer" && dotnet build -v quiet --nologo ) > "$BUILD_LOG" 2>&1; then
    ...
    rm -f "$BUILD_LOG"
    dotnet run --no-build --project "$ROOT_DIR/tools/dotnet/analyzer" -- --src "$SOURCE_CODE_PATH" --out "$ROSLYN_JSON"
...
( cd "$ROOT_DIR/tools/grammar" && python3 -m grammarc.cli "${GRAMMARC_ARGS[@]}" )
