...
services) used by `scripts/e2e/e2e-test.sh` / `.github/workflows/e2e.yml` (Top-20 #7) to
...
(a genuinely healthy target) as degraded. `scripts/e2e/e2e-test.sh` failed immediately with
...
(`eshprep/src/PublicApi/`, `bitwarden_prep/src/Api/`, etc). `bin/fuzz-prep-multi.py`'s
...
Same commands a real target would use — see `scripts/e2e/e2e-test.sh` for the exact sequence
this fixture is exercised with in CI:

```bash
python3 bin/fuzz-prep-multi.py --src fixtures/planted-bug-api --out /tmp/planted-bug-prep --main PlantedBugApi
cd /tmp/planted-bug-prep && docker compose build && docker compose up -d
# ... bin/compile-grammar.sh, void -direct-shm, etc.
```
