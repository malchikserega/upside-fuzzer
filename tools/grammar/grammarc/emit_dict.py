...
# bin/compile-grammar.sh run, so it is not a safe place to hand-edit values. This is
...
_CUSTOM_DICT_SCAFFOLD: Dict[str, Any] = {
    "_readme": [
        "This file is yours. bin/compile-grammar.sh reads and merges it into dict.json",
        "on every run, but NEVER overwrites or deletes it -- values you add here",
        "survive re-running bin/compile-grammar.sh, including after the target's API changes.",
        "",
