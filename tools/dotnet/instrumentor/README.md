...
directly by a human in the normal pipeline — `tools/prep/fuzzprep/` bakes a copy of this project's
...
| File | Purpose |
|---|---|
| `Program.cs` | Everything: CLI parsing, `NamespaceMatcher`/`InstrumentationFilter` (type-selection logic), SharpFuzz's basic-block coverage rewrite, `CmpLogInstrumentor` (records live string/int comparison operands — CmpLog/RedQueen, see `docs/architecture/engine.md` §"CmpLog/RedQueen"), and `ConstantExtractor` (read-only harvest of string/int literals at instrument time). |
...
`instrumentor_gen.py`/`coverage_helper_gen.py` in `tools/prep/fuzzprep/` are what actually copy this
project's source into a target's Docker build context; see `tools/prep/fuzzprep/__init__.py`'s
module-map docstring if you're tracing that path instead of this tool's own logic.

## Build & test

```bash
dotnet build tools/dotnet/instrumentor/instrumentor.csproj
