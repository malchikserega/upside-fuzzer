...
`docs/development/architecture-review.md`'s Grammar Generation section for the tradeoff this buys).

Invoked by `bin/compile-grammar.sh` whenever `--src` is given; not run directly in the normal
pipeline. Output is `roslyn-constraints.json`, the JSON contract `tools/grammar/grammarc/roslyn_merge.py`
reads and merges over the OpenAPI-derived grammar (Roslyn wins per scoped field).

## Usage

```bash
dotnet run --project tools/dotnet/analyzer/ -- --src <source-dir> --out roslyn-constraints.json [--verbose]
```
...
```bash
dotnet build tools/dotnet/analyzer/analyzer.csproj
