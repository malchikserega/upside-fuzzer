...
tools/dotnet/instrumentor/Program.cs) plus the namespaces.json config it reads, and
...
def generate_unified_instrumentor(result: MultiAnalysisResult, output_path: Path):
    ...
    # Copy the generic instrumentor from tools/dotnet/instrumentor/ (this file
    # lives at tools/prep/fuzzprep/instrumentor_gen.py -- three levels down).
    repo_instrumentor = Path(__file__).parent.parent.parent / "dotnet" / "instrumentor" / "Program.cs"
...
_COVERAGE_HOOK_CS = r'''// AUTO-GENERATED by bin/fuzz-prep-multi.py — UpsideFuzz zero-edit coverage runtime.
//
