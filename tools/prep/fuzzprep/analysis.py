...
class MultiProjectAnalyzer:
    ...
    def _extract_assembly_name(self, csproj_content: str, csproj_stem: str) -> str:
        """Extract AssemblyName from csproj, falling back to project name.

        Most projects never set this and the build output DLL matches the
        project name -- but it's a real, unremarkable MSBuild property to
        override (seen on Jellyfin: Jellyfin.Server.csproj sets
        <AssemblyName>jellyfin</AssemblyName>, so the actual build output is
        jellyfin.dll, not Jellyfin.Server.dll). Every codepath that constructs
        an on-disk .dll path must use this, not the bare project name.
        """
        m = re.search(r'<AssemblyName>(.*?)</AssemblyName>', csproj_content)
        return m.group(1).strip() if m else csproj_stem
    ...
    def analyze_solution(self, manual_main: Optional[str] = None) -> MultiAnalysisResult:
        ...
        for csproj in csproj_files:
            ...
            assembly_name = self._extract_assembly_name(content, csproj.stem)
