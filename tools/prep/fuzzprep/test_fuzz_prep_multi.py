...
what used to be one ~2.8k-line bin/fuzz-prep-multi.py) is the single most fragile
...
hand while building fixtures/demo-app/ against it in one session: non-main-project
DLLs shipped uninstrumented, _detect_last_stage misidentifying the runtime
stage on an unnamed final FROM (reproduced concretely on fixtures/demo-app/Dockerfile,
...
Run with: cd tools/prep && python3 -m unittest fuzzprep.test_fuzz_prep_multi -v
...
class DetectLastStageTests(unittest.TestCase):
    def test_unnamed_final_stage_returns_none_not_an_earlier_stage(self):
        # The exact shape of fixtures/demo-app/Dockerfile: a named build stage, then an
...
class AssemblyNameTests(unittest.TestCase):
    """A fourth real bug found the same way as the three in this file's module
    docstring, this time building against jellyfin/jellyfin: every Dockerfile-
    generation codepath assumed a project's build output is always
    `{project_name}.dll`, which is false whenever a .csproj overrides
    <AssemblyName> (Jellyfin.Server.csproj sets it to `jellyfin`, so the real
    output is jellyfin.dll -- the previous code generated a `RUN dotnet
    .../instrumentor.dll /src/.../Jellyfin.Server.dll ...` instrumentation
    step against a file that doesn't exist, failing the Docker build).
    """

    def test_extract_assembly_name_reads_the_override(self):
        analyzer = MultiProjectAnalyzer("/nonexistent")
        content = "<Project><PropertyGroup><AssemblyName>jellyfin</AssemblyName></PropertyGroup></Project>"
        self.assertEqual(analyzer._extract_assembly_name(content, "Jellyfin.Server"), "jellyfin")

    def test_extract_assembly_name_falls_back_to_project_stem(self):
        analyzer = MultiProjectAnalyzer("/nonexistent")
        self.assertEqual(analyzer._extract_assembly_name("<Project></Project>", "TeamFlow.Api"), "TeamFlow.Api")

    def test_project_info_dll_name_uses_assembly_name_override(self):
        proj = ProjectInfo(
            name="Jellyfin.Server", path=Path("Jellyfin.Server"), csproj_path=Path("x.csproj"),
            is_web=True, target_framework="net10.0", namespaces=[], business_logic_files=[],
            assembly_name="jellyfin",
        )
        self.assertEqual(proj.dll_name, "jellyfin.dll")

    def test_project_info_dll_name_defaults_to_project_name(self):
        proj = ProjectInfo(
            name="TeamFlow.Api", path=Path("TeamFlow.Api"), csproj_path=Path("x.csproj"),
            is_web=True, target_framework="net8.0", namespaces=[], business_logic_files=[],
        )
        self.assertEqual(proj.dll_name, "TeamFlow.Api.dll")
