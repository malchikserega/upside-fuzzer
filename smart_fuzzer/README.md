...
GOOS=darwin  GOARCH=arm64 go build -o ../smartfuzzergo-darwin-arm64 .

# macOS Intel
GOOS=darwin  GOARCH=amd64 go build -o ../smartfuzzergo-darwin-amd64 .

# Linux aarch64 (ARM64 — Docker on M1, AWS Graviton, RPi)
GOOS=linux   GOARCH=arm64 go build -o ../smartfuzzergo-linux-arm64 .

# Linux amd64 (most servers, Docker on Intel/AMD)
GOOS=linux   GOARCH=amd64 go build -o ../smartfuzzergo-linux-amd64 .

# Windows 64-bit
GOOS=windows GOARCH=amd64 go build -o ../smartfuzzergo-windows-amd64.exe .
```
...
docker run --rm -v "$(pwd)/go:/src" -w /src golang:1.22-alpine \
  go build -o /src/smartfuzzergo-linux-amd64 .

# macOS arm64 cross-compiled inside Docker
docker run --rm -v "$(pwd)/go:/src" -w /src golang:1.22-alpine \
  sh -c "GOOS=darwin GOARCH=arm64 go build -o /src/smartfuzzergo-darwin-arm64 ."
```
...
WORKDIR /src
COPY go/ .
RUN go build -ldflags="-s -w" -o /smartfuzzergo .

FROM alpine:3.19
COPY --from=builder /smartfuzzergo /smartfuzzergo
COPY export-templates.py /export-templates.py
ENTRYPOINT ["/smartfuzzergo"]
```
...
docker buildx build \
  ...
  -f smart_fuzzer/Dockerfile.go \
  -t smartfuzzergo:latest \
  smart_fuzzer/
```
