...
Security-focused flag interactions and recommended profiles: **[`INSTRUCTIONS.md`](../docs/INSTRUCTIONS.md#security-campaign-profiles)**.
...
|------|---------|-------------|
| `-grammar` | `.` | Directory with `templates.export.json` and `dict.json` (written directly by `compile-grammar.sh` — see [ARCHITECTURE.md §6](../docs/ARCHITECTURE.md)) |
...
— populated by `grammarc`/`analyzer` from the target's OpenAPI spec + real C# validation
