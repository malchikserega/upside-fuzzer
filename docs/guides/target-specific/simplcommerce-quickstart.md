...
SimplCommerce is a highly modular application. We provide a `simpl_namespaces.json` file in the `docs/guides/examples/simplcommerce/` directory to instruct the instrumentor to only track coverage in the core modules and avoid noisy third-party dependencies.
...
python3 bin/fuzz-prep-multi.py \
  ...
  --namespaces docs/guides/examples/simplcommerce/simpl_namespaces.json
```
...
SimplCommerce's automatically generated Swagger definition contains some redundant or invalid path parameters that a strict OpenAPI parser (including `tools/grammar/grammarc/oas.py`, RESTler's first-party replacement — see `ARCHITECTURE_REVIEW.md`'s Grammar Generation section) chokes on.
...
cd ..
python3 docs/guides/examples/simplcommerce/fix_swagger_paths.py
```
...
`tools/grammar/grammarc/` + `tools/dotnet/analyzer/` (one command, no Docker), writing `templates.export.json` +
`dict.json` straight to `grammars/simplcommerce/`:

```bash
./bin/compile-grammar.sh simplcommerce_prep/swagger-sanitized.json --src ./simplcommerce --out grammars/simplcommerce
```
...
python3 ../docs/guides/examples/simplcommerce/get_cookie.py
```
...
**→ [Back to README](../../../README.md) · [Full Runbook](../../getting-started/quickstart.md) · [SimplCommerce Report](../../reports/SIMPLCOMMERCE_REPORT.md)**
