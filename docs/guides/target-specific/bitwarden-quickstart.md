...
> [BITWARDEN_FUZZ_RUNBOOK.md](bitwarden-runbook.md) Steps 1-6 for the current,
...
> **Re-verified end-to-end from a completely fresh clone on 2026-07-23**: `git clone` → `bin/fuzz-prep-multi.py` → `docker compose build --no-cache` → bring up → `bin/verify-hook.sh` → `bin/compile-grammar.sh` → fuzz, twice (once in each run mode below). Grammar output was byte-for-byte identical to prior runs (595 operations, 599 templates, 0 skipped). This pass also found and fixed two real fuzzer-engine bugs (a path-quoting leak and a malformed-auth-token leak on unauthenticated probes) — see `ARCHITECTURE_REVIEW.md`'s Inconsistencies section for the writeups. Every command below is exactly what was run, not aspirational.

**→ [Back to README](../../../README.md) · [Full Runbook](../../getting-started/quickstart.md) · [Authentication Guide](../authentication.md)**

> **Already have a working `bitwarden_prep/` checkout?** Use [BITWARDEN_FUZZ_RUNBOOK.md](bitwarden-runbook.md) instead — it skips straight to bring-up/iterate/re-run and documents every gotcha found during setup. Come back to *this* guide for a fresh, from-scratch setup or if you're setting Bitwarden up for the first time.
...
Use the root-level `bin/verify-hook.sh` — it's the more thorough check (8 assertions: SHM
...
```bash
cd ..  # repo root
BASE_URL=http://localhost:4000 PROBE=/accounts/profile ./bin/verify-hook.sh
...
```bash
cd bitwarden_prep
python3 ../docs/guides/examples/bitwarden/get_apikey.py
```
...
For a multi-user access-control campaign, run `get_apikey.py` for a second user, then manually add a second identity entry. See [`FUZZER_AUTHENTICATION.md`](../authentication.md) for the full auth file schema.
...
python3 ../docs/guides/examples/bitwarden/populate_data.py --auth-file auth.identities.json

# Single-user fallback (reads fuzzer.env)
python3 ../docs/guides/examples/bitwarden/populate_data.py
```
...
python3 ../docs/guides/examples/bitwarden/populate_data.py --auth-file auth.identities.json --identity bitwarden-user

# Control object counts
python3 ../docs/guides/examples/bitwarden/populate_data.py --auth-file auth.identities.json --folders 5 --ciphers 30 --sends 10

# Dry-run to preview what would be created
python3 ../docs/guides/examples/bitwarden/populate_data.py --auth-file auth.identities.json --dry-run
```
...
RESTler is retired — grammar compilation is now one first-party command (`tools/grammar/grammarc/` +
`tools/dotnet/analyzer/`), no Docker involved. The sanitize step below is **still needed**: it's not
a RESTler-specific workaround, it's because `tools/grammar/grammarc/oas.py` doesn't yet decompose
...
# Sanitize: Bitwarden uses deepObject/nested $ref query params tools/grammar/grammarc/ doesn't yet decompose
cd bitwarden_prep
python3 ../docs/guides/examples/bitwarden/sanitize_swagger.py internal_swagger.json
cd ..

# Compile grammar (tools/grammar/grammarc/ OpenAPI parser + tools/dotnet/analyzer/ Roslyn syntax-tree analysis) — one
# command, writes templates.export.json + dict.json directly to grammars/bitwarden/
./bin/compile-grammar.sh bitwarden_prep/internal_swagger.json --src ./bitwarden_src --out grammars/bitwarden
```
...
[tools/grammar/grammarc/] operations=595 templates=599 skipped=0 multipart_endpoints=4 roslyn_matched_types=284 dict_keys=845 -> grammars/bitwarden
```
...
> **Tip:** If you have a security-specific dictionary overlay (`dict.security.json`), pass it directly: `./bin/compile-grammar.sh ... --dict dict.security.json`. The security overlay adds Bitwarden-specific payloads for `returnUrl`, `redirect_uri`, device identifiers, base64url tokens, and GUID-heavy paths.
...
Both modes accept the same CLI flags (see [void/README.md](../../../src/void/cmd/void/README.md) for the full
reference) — pick whichever fits your workflow. You don't need to run both.

### Mode A — Host mode (no image build, HTTP coverage)

```bash
go -C /path/to/upside-fuzzer/src/void build -o /tmp/smartfuzzergo ./cmd/void
cd ../../bitwarden_prep
...
> (`shouldUseMmap()`, `src/void/internal/engine/coverage.go`) — which is exactly what this container is,
...
`coverage_end_edges` climbed to ~51,500 (real, nonzero — confirmed via `bin/verify-hook.sh`
...
[CLI.md](../../getting-started/cli.md) for the full subcommand reference.
...
| Rebuilding after a fresh `bitwarden_src` clone and the API build fails to find `Api.dll`/`Core.dll`/etc. to instrument (`skipped` on every DLL) | Your `bin/fuzz-prep-multi.py` predates the `PublishSingleFile`-stripping and multi-line `-o`-detection fixes (2026-07-22). Pull latest and regenerate — the tool now prints `Detected PublishSingleFile=true — disabled...` and the instrumentation RUN commands should reference the real publish dir (e.g. `out/Api.dll`, not a guessed `/app/publish/Api.dll`). |
...
| Lots of `needs_review`/`noise` crashes with "Unrecognized Guid format" and a literal `"` inside the path (e.g. `/organizations/"CIP-0042"/delete`) | Fixed 2026-07-23 (`tools/grammar/grammarc/body_serializer.py`'s path/query/header segments were reusing the JSON-body quoting logic). `git pull` and regenerate the grammar (Step 7) if you still see this. |
...
| Fuzzer exits immediately with `run failed: coverage instrumentation degraded: ...` | Added 2026-07-23 (Top-20 #4) — the engine now sends a real warm-up probe at startup and refuses to run if the coverage bitmap doesn't move. This means instrumentation is genuinely broken (not a false alarm): re-check Step 3's `bin/verify-hook.sh` output, confirm `api`/`identity` were rebuilt from the latest `bin/fuzz-prep-multi.py`, and don't just pass `-allow-degraded-coverage` — that run would complete but find nothing. |

---

**→ [Back to README](../../../README.md) · [Full Runbook](../../getting-started/quickstart.md) · [Authentication Guide](../authentication.md) · [Architecture](../../architecture/overview.md)**
