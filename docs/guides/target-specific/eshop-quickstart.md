...
```bash
cd ..  # repo root
BASE_URL=http://localhost:5200 PROBE=/api/catalog-items ./bin/verify-hook.sh
```
...
# Compile grammar (tools/grammar/grammarc/ OpenAPI parser + tools/dotnet/analyzer/ Roslyn syntax-tree analysis of ./esh)
./bin/compile-grammar.sh swagger-eshop.json --src ./esh --out grammars/eshop
```
...
[tools/grammar/grammarc/] operations=8 templates=8 skipped=0 multipart_endpoints=0 roslyn_matched_types=3 dict_keys=25 -> grammars/eshop
```
...
See [CLI.md](../../getting-started/cli.md) for the full subcommand reference, and its "The `localhost`
...
| Step | Command |
|------|---------|
| Instrument | `python3 bin/fuzz-prep-multi.py --src ./esh --out ./eshprep --main PublicApi` |
...
| Verify hook | `cd .. && BASE_URL=http://localhost:5200 PROBE=/api/catalog-items ./bin/verify-hook.sh` |
| Grammar | `./bin/compile-grammar.sh swagger-eshop.json --src ./esh --out grammars/eshop` (one command, no Docker/RESTler) |
...
| `go.sum not found` on Docker build | Use the current repo checkout; `go.sum` is tracked and should already be present |
...
| Want to confirm the hook is wired before a full fuzzing run | `BASE_URL=http://localhost:5200 PROBE=/api/catalog-items ./bin/verify-hook.sh` from the repo root — runs the 5-point smoke test in [Step 4](#step-4-verify-coverage-instrumentation) automatically. |
