...
> After that, open `grammars/eshop/dict.custom.json` (create it by running `bin/compile-grammar.sh` once first if it doesn't exist yet) and add these values under the matching field-name keys (`productId`, `categoryId`, `userEmail`), keeping the existing `_readme` key as-is."

## Step 3: How to Populate `dict.custom.json`

`dict.custom.json` is a **flat map**, identical in format to `dict.json` (the auto-generated file it gets merged into — see [INSTRUCTIONS.md §10](../getting-started/quickstart.md#10-custom-dictionary-format)): each key is a request field/payload name, each value an array of candidate strings. `src/void/internal/engine/store.go::candidatesForKey` matches keys case-insensitively with canonical normalization, so `productId`, `ProductID`, and `product_id` all resolve to the same pool — you don't need multiple casings.

**Example BEFORE enrichment** (the starter scaffold `bin/compile-grammar.sh` writes the first time — its `_readme` key is safe to leave in place, the engine never looks up a field literally named `_readme`):
```json
{
...
*Legacy note: older dictionaries used a one-level-nested RESTler-shaped format (`{"restler_custom_payload": {"productId": [...]}}`). That format still works as `--dict` input — `tools/grammar/grammarc/emit_dict.py::merge_external_dict` flattens it automatically — but `bin/compile-grammar.sh` never generates it anymore, and there's no reason to write it by hand today. Use the flat format above.*

## Step 4: Run — one caveat depending on which file you edited

The Go engine only ever reads `dict.json` at startup — `src/void/internal/engine/store.go::loadDict` is called fresh each time from the path you pass via `-dict` or `<grammar-dir>/dict.json`. It never reads `dict.custom.json` directly. That gives you two options, trading off speed against durability:

| You edited... | To pick it up | Survives the next `bin/compile-grammar.sh`? |
|---|---|---|
...
| `dict.custom.json` | Run `bin/compile-grammar.sh` once more first (fast, no Docker — it only re-derives `templates.export.json`/`dict.json` from the spec) to merge it in, *then* start the fuzzer | **Yes.** This is the durable path — recommended for anything you'll want again. |

```bash
# Durable path: merge dict.custom.json into a fresh dict.json, then run
./bin/compile-grammar.sh swagger.json --out grammars/eshop
...
If you don't see the value count increase, double check you edited the file the running container actually mounts (`-grammar`'s directory), not a stale copy elsewhere — and, if you edited `dict.custom.json`, that you re-ran `bin/compile-grammar.sh` before starting the fuzzer.
...
This runs cleanly in CI/CD pipelines where the dictionary is always fresh and relevant to the current state of the test database: enrich `dict.custom.json` from the database, then run `bin/compile-grammar.sh` (which merges it into a freshly-regenerated `dict.json`) before each fuzzing run.
