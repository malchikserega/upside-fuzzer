...
A single orchestrator over the existing pipeline — `bin/fuzz-prep-multi.py` → `docker compose` →
`bin/verify-hook.sh` → `bin/compile-grammar.sh` → `void` — so you don't have to remember four
...
directly, exactly as documented in [INSTRUCTIONS.md](quickstart.md) and the
...
|------|-------------------|---------|
| **Native** | Python 3 (always); Docker (build/up/down/fuzz `--direct-shm`); .NET SDK 8+ (only if you pass `--src` for Roslyn constraints) | `python3 bin/compatibility/upsidefuzz.py <subcommand> ...` |
| **Zero-install (Docker)** | Docker only | `./upsidefuzz <subcommand> ...` |

The zero-install mode runs `bin/compatibility/upsidefuzz.py` inside `deployments/docker/Dockerfile.cli`, an image that bundles
...
python3 bin/compatibility/upsidefuzz.py run --src ~/my-target/src --out ./my-target-fuzz --main MyProject.Api \
  --target http://localhost:8080 --swagger http://localhost:8080/swagger/v1/swagger.json \
  --time-budget 15
```
...
|---|---|---|
| `instrument` | `bin/fuzz-prep-multi.py` | Produce a zero-edit instrumented copy of a .NET solution |
...
| `verify` | `bin/verify-hook.sh` | Confirm the coverage hook is actually linked and producing edges |
| `grammar` | `bin/compile-grammar.sh` | Compile `templates.export.json` + `dict.json` from a Swagger spec (+ optional Roslyn) |
...
Same flags as `bin/fuzz-prep-multi.py --help` (`--exclude-namespaces`, `--inject-mode
hook|source`, `--no-compose`). See [INSTRUCTIONS.md §3](quickstart.md#3-step-1-instrument-the-project).
...
[QUICKSTART_BITWARDEN.md](../guides/target-specific/bitwarden-quickstart.md) instead — `up` is for the common,
single-stage case.
...
Thin wrapper over `bin/verify-hook.sh` — same `--up`/`--down`/`--dir`/`--probe`/`--base` flags,
same 5-check output (SHM create, `/shm/health`, per-request attribution, real-traffic
coverage growth, cumulative edges). See [ARCHITECTURE.md §5](../architecture/overview.md).
...
Same as `bin/compile-grammar.sh`, plus one convenience: the swagger argument may be an
...
[void/README.md](../../src/void/cmd/void/README.md) is always available, nothing is hidden.

The `void` binary itself is resolved in this order: `--void-bin` if given, then
`src/void/cmd/void/void` (the native build convention), then `void` on `PATH` (what the Docker image
provides). If none is found, native mode tells you to `go -C src/void build -o cmd/void/void ./cmd/void`
...
- a `void` binary, built from `src/void/` in the same multi-stage build `deployments/docker/Dockerfile.cli`
  uses
...
- this repo's own tooling (`bin/fuzz-prep-multi.py`, `bin/compile-grammar.sh`, `bin/verify-hook.sh`,
  `tools/grammar/grammarc/`, `tools/dotnet/analyzer/`), baked in at `/upsidefuzz`
...
`bin/compatibility/upsidefuzz.py`'s `_containerize_url` transparently rewrites any `localhost`/`127.0.0.1` in
...
| Target | CLI section |
|---|---|
| fixtures/demo-app/ (TeamFlow, in-repo) | [fixtures/demo-app/README.md § The same thing, via the upsidefuzz CLI](../../fixtures/demo-app/README.md#the-same-thing-via-the-upsidefuzz-cli) — `run` works for a quick/guest-only pass; per-identity auth needs the individual subcommands, token collection in between |
| eShopOnWeb | [QUICKSTART_ESHOP.md § The same thing, via the upsidefuzz CLI](../guides/target-specific/eshop-quickstart.md#the-same-thing-via-the-upsidefuzz-cli) — `run` works end to end, no non-standard steps |
| BTCPayServer | [QUICKSTART_BTCPAYSERVER.md § The same thing, via the upsidefuzz CLI](../guides/target-specific/btcpayserver-quickstart.md#the-same-thing-via-the-upsidefuzz-cli) — basic-auth swagger download stays manual |
| SimplCommerce | [QUICKSTART_SIMPLCOMMERCE.md § The same thing, via the upsidefuzz CLI](../guides/target-specific/simplcommerce-quickstart.md#the-same-thing-via-the-upsidefuzz-cli) — swagger sanitize + cookie login stay manual |
| Bitwarden (fresh) | [QUICKSTART_BITWARDEN.md § The same thing, via the upsidefuzz CLI](../guides/target-specific/bitwarden-quickstart.md#the-same-thing-via-the-upsidefuzz-cli) — multi-stage bring-up (mssql → migrator → api/identity) stays manual |
| Bitwarden (already set up) | [BITWARDEN_FUZZ_RUNBOOK.md § 8](../guides/target-specific/bitwarden-runbook.md#8-the-same-thing-via-the-upsidefuzz-cli) |
...
| Symptom | Cause / fix |
|---|---|
| `Could not find a void binary` | Native mode only: `go -C src/void build -o cmd/void/void ./cmd/void`, or use `./upsidefuzz` instead |
| `run failed: coverage instrumentation degraded: ...` | The fail-closed startup check (Top-20 #4) caught real instrumentation breakage — see [ARCHITECTURE_ENGINE.md](../architecture/engine.md)'s "Self-verifying, fail-closed instrumentation" section, don't just pass `-allow-degraded-coverage` |
...
| Docker-mode `grammar`/`run` writes files that "disappear" | Fixed 2026-07-24 — `bin/compile-grammar.sh` used to resolve relative `--out`/`--swagger` paths against its own script location rather than your cwd, which only happened to be harmless when they were the same directory. `git pull` if you still see this. |
