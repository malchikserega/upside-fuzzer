...
**→ [Back to README](../../README.md) · [Installation/Prerequisites](installation.md) · [Architecture](../architecture/overview.md) · [Authentication](../guides/authentication.md) · [Docs Index](../index.md)**

---

Before this: [Installation / Prerequisites](installation.md) (Docker, Python, .NET SDK, optional Go).
...
(Numbering starts at 2 deliberately — section 1, Prerequisites, is now
[installation.md](installation.md); every in-page cross-reference below
that cites a section by number was left as-is rather than renumbered.)

**Project quickstarts:** [Bitwarden](../guides/target-specific/bitwarden-quickstart.md) · [BTCPayServer](../guides/target-specific/btcpayserver-quickstart.md) · [eShopOnWeb](../guides/target-specific/eshop-quickstart.md) · [SimplCommerce](../guides/target-specific/simplcommerce-quickstart.md)
...
└────────────────────────────┬────────────────────────────────────────────┘
                             │
              bin/compile-grammar.sh swagger.json --src <src>
              (tools/grammar/grammarc/ OpenAPI parser + tools/dotnet/analyzer/ Roslyn syntax-tree
...
```bash
python3 /path/to/upside-fuzzer/bin/fuzz-prep-multi.py \
  --src <SOURCE_DIR> \
  --out <OUTPUT_DIR> \
...
Full mechanism in `ARCHITECTURE_ENGINE.md` ("CmpLog/RedQueen via IL Comparison
Instrumentation").

New command (explicit, equivalent to the default):
```bash
python3 bin/fuzz-prep-multi.py --src <SOURCE_DIR> --out <OUTPUT_DIR> --main <PROJECT> --inject-mode hook
```
Legacy behavior (previous releases):
```bash
python3 bin/fuzz-prep-multi.py --src <SOURCE_DIR> --out <OUTPUT_DIR> --main <PROJECT> --inject-mode source
```
...
7. **Compose file** — two cases:
   ...
   - **No compose file found** (true for many multi-service targets — Bitwarden's own repo has
     ...
     definition). See `docs/guides/target-specific/bitwarden-runbook.md` §2 for a complete worked example.
...
- Wrong project `--main` → re-run `bin/fuzz-prep-multi.py` pointing to the correct web project
...
./bin/compile-grammar.sh swagger.json --src <SOURCE_DIR> --out grammars/<project>

# Or with a custom dictionary for domain-specific values:
./bin/compile-grammar.sh swagger.json --dict custom-dict.json --src <SOURCE_DIR> --out grammars/<project>
```
...
### What `bin/compile-grammar.sh` does
...
1. **`tools/grammar/grammarc/`** (Python, stdlib-only) parses the OpenAPI spec directly ($ref/allOf/oneOf/anyOf
   ...
   to segments.
2. **`tools/dotnet/analyzer/`** (C#, real `Microsoft.CodeAnalysis.CSharp` syntax-tree parsing — not regex),
...
3. `tools/grammar/grammarc/roslyn_merge.py` merges the two, Roslyn winning per-field on a scoped match, and
...
directory passed via `-grammar` — `bin/compile-grammar.sh` writes both there already, no
...
regenerated: `src/void/internal/engine/template.go`'s `exportTemplates()` invokes it automatically at Void
startup only if `templates.export.json` is missing but a `grammar.py` is present. If you
regenerate with the current `bin/compile-grammar.sh`, you'll never hit this path. If you do
see `exec: "python3": executable file not found in $PATH` against an old grammar
directory, either regenerate it with `bin/compile-grammar.sh` (recommended — removes the
...
> For this to work, the `void` service must be defined in the compose file with the `fuzz-go` profile. `bin/fuzz-prep-multi.py` adds a commented-out template — uncomment and configure it.
...
See **[`FUZZER_AUTHENTICATION.md`](../guides/authentication.md)** and **[`auth.identities.example.json`](../guides/auth.identities.example.json)**.
...
| **`AUTH_URL`**, **`AUTH_METHOD`**, **`AUTH_BODY`**, **`AUTH_CONTENT_TYPE`**, **`AUTH_TOKEN_FIELD`** | If no token/headers/cookie are pre-set, Void can perform one login request against `TARGET_HOST` and parse a token (defaults in code: `POST` `/api/authenticate`, JSON field `token`). See `src/void/internal/engine/auth.go`. |
...
> **Defaults shown below are for orientation only — [`void/README.md`](../../src/void/cmd/void/README.md)
> is the single source of truth for current defaults (this file doesn't restate every
> value to avoid the two drifting apart).**
...
| Feature | To disable |
|---------|------------|
| Adaptive concurrency | `-adaptive-concurrency=false` |
| Crash triage & scoring | `-crash-triage=false` |
| Crash reproducibility check | `-repro-runs 0` |
| Crash payload minimization | `-minimize-crash=false` |
| Race condition detection | `-race-mode=false` |
| Anti-forgery token harvesting | `-auto-antiforgery=false` |
| Source-aware endpoint priority | `-source-aware-priority=false` |
| Multi-identity scheduling | `-multi-identity=false`; configure identities with `-auth-file` |
...
| Flag | When to change |
|------|----------------|
| `-time-budget` | Increase for a thorough scan (60–120 min; default is a short 10-minute smoke run) |
| `-concurrency` / `-max-concurrency` | Increase for fast targets (32–64 / up to 128) |
| `-request-timeout` | Lower for fast targets (2.5s), raise for slow ones |
| `-direct-shm` | Set `true` in Docker sidecar mode |
| `-auth-file` | Use for JWT/API-key/cookie multi-identity access-control fuzzing |
| `-sequence-prob` | Raise to 0.5–0.6 for stateful API testing |
| `-sequence-max-depth` | Raise to 5–6 for deep workflows |
| `-sequence-fanout` | Raise to 8–10 for wide API surface |
| `-skip-endpoint-on-500` | Set `true` when 500s are expected (misconfigured infra) |
| `-coverage-interval` | Raise to 5–10 for throughput benchmarking |
| `-cmplog` | Set `false` to skip polling `/shm/cmplog` (no effect against a target instrumented without `--cmplog`, or in `--inject-mode source`) |
| `-cmplog-interval` | Raise on very high-latency targets to cut poll overhead |
...
See [`void/README.md`](../../src/void/cmd/void/README.md) for the complete CLI table with defaults taken from source.

### Build for any platform

See [`void/README.md §Build`](../../src/void/cmd/void/README.md#build-for-any-platform) for cross-compilation and Docker buildx instructions.
...
See [QUICKSTART_BITWARDEN.md](../guides/target-specific/bitwarden-quickstart.md).
...
See [QUICKSTART_BTCPAYSERVER.md](../guides/target-specific/btcpayserver-quickstart.md).
...
See [QUICKSTART_ESHOP.md](../guides/target-specific/eshop-quickstart.md).
...
See [QUICKSTART_SIMPLCOMMERCE.md](../guides/target-specific/simplcommerce-quickstart.md).
...
Every `bin/compile-grammar.sh` run writes two dictionary files into the grammar's `--out` directory, and they have opposite lifecycles — knowing which is which is the whole trick to using this effectively:

| File | Written by | Lifecycle | Edit it? |
|------|-----------|-----------|----------|
| `dict.json` | `tools/grammar/grammarc/`, auto-discovered from the target's own OpenAPI spec (+ Roslyn constraints, if `--src` was given) | **Fully overwritten every compile.** | No — hand edits here are silently lost on the next run. |
| `dict.custom.json` | **You.** `tools/grammar/grammarc/` writes a starter copy the first time it compiles a given `--out` directory | **Never touched again once it exists** — `tools/grammar/grammarc/` only ever reads it (to merge into `dict.json`), never writes to it | **Yes — this is the file.** |

### The workflow

1. Run `bin/compile-grammar.sh` once. If `dict.custom.json` doesn't exist yet in `--out`, `tools/grammar/grammarc/` creates a starter copy with an inline `_readme` (plain JSON strings — the file stays valid JSON, since real comments aren't legal there) and one obviously-fake `exampleFieldName` key to show the shape.
2. Open the freshly-written `dict.json` in the same directory to see the real field/payload keys `tools/grammar/grammarc/` discovered for this target (`currencyCode`, `userId`, `amount`, ...).
...
4. Re-run `bin/compile-grammar.sh` (e.g. after the target's API changes) as often as you like — `dict.custom.json` is read and merged into the fresh `dict.json` every time, and is itself never regenerated or overwritten. Your additions survive indefinitely.
...
`tools/grammar/grammarc/emit_dict.py` writes (and `src/void/internal/engine/store.go` reads) a simple **flat map**: each key is a request field/payload name, each value an array of candidate strings. `dict.custom.json` uses the identical format.

### Structure (recommended — what `tools/grammar/grammarc/` itself emits)
...
normalization (`src/void/internal/engine/store.go::candidatesForKey`) — `currencyCode`, `currency_code`,
and `CurrencyCode` all resolve to the same pool.

### Legacy nested containers (still supported, not required)

For backward compatibility, `src/void/internal/engine/store.go` also special-cases exactly four
...
`tools/grammar/grammarc/` never emits this nested shape itself (flat keys are simpler and match the
same lookup path) — but if you pass `--dict` pointing at an old dictionary that uses it,
`tools/grammar/grammarc/emit_dict.py::merge_external_dict` flattens it into top-level keys automatically,
...
- **Two paths reach the same pool now.** A field's boundary values land in `dict.json`
  (via `tools/grammar/grammarc/boundary.py`) *and*, since Top-20 #14, directly on the segment itself
  ...
  to choose between them — both are populated automatically by `bin/compile-grammar.sh`.
...
Before trusting a change to `tools/grammar/grammarc/`, `tools/dotnet/analyzer/`, or the mutation engine, run the
...
This instruments the fixture, brings up the container, runs `bin/verify-hook.sh`, compiles
...
| `/shm/create` returns 404 | Wrong project instrumented | Re-run `bin/fuzz-prep-multi.py --main <correct-project>` |
...
| Fuzzer exits immediately | Malformed `templates.export.json`/`dict.json` | Regenerate with `bin/compile-grammar.sh` (check its console output for `skipped=N > 0`); validate JSON with `python3 -m json.tool grammars/<project>/templates.export.json > /dev/null` |
| `exec: "python3": executable file not found` inside Void | Only hit against a **pre-migration** grammar directory (has `grammar.py`, no `templates.export.json`) — Void's legacy `export-templates.py` fallback needs `python3` | Regenerate the grammar with the current `bin/compile-grammar.sh` (removes the dependency entirely — the primary path never calls `export-templates.py`), or rebuild `void/Dockerfile.go`, which still bundles `python3` for this fallback |
| Void exits at startup (templates JSON missing / load error) | `-grammar`/`-templates-json` points at a directory without `templates.export.json` | Re-run `bin/compile-grammar.sh <swagger> --out <that directory>`; it writes `templates.export.json` there directly — no separate export step needed |

---

**→ [Back to README](../../README.md) · [Architecture](../architecture/overview.md) · [Authentication](../guides/authentication.md) · [Go Fuzzer Reference](../../src/void/cmd/void/README.md)**
