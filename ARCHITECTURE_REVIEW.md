...
2. **[MED] Mutation is value-centric, not structure- or grammar-aware.** `mutation_engine.go` mutates rendered strings/JSON after the template is flattened to a raw byte request (`renderTemplate` → `parseRawRequest`). It has thrown away the typed schema by then. It cannot target "the field the server just complained about," cannot do coverage-directed *field* selection, and cannot do dictionary-token-from-comparison (AFL's `CMPLOG`/RedQueen — extracting magic values from failed comparisons). No input-to-state / comparison feedback exists.
...
| 12 | **State-reward sequence search** (coarse state signature; reward new states) | P1 | Med–High | 3 wk | Med | High | — | The DeepREST/EvoMaster gap for logic bugs |
...
| 17 | **Bitmap sizing from real instrumented-edge count; drop reset hack; true coverage %** | P2 | Low | 3–5 d | Med | — | — | Honest metrics; fewer collisions on big apps |
| 18 | **Differential oracles** (verb/param-location/content-type/route-case confusion) | P2 | Med | 2 wk | — | Med | — | New auth-bypass & parser-confusion bug class |
| 19 | **Single `upsidefuzz` CLI orchestrator** (prep→grammar→fuzz one command) + repo hygiene | P2 | Med | 2 wk | — | — | High | Adoption; hides 4-language pipeline |
...
**Phase 4 — Product polish (P2):** persistent/resumable corpus (#15), SARIF/HTML (#16), honest coverage metrics (#17), single CLI + packaging + releases (#19), value-level minimization (#20). *Outcome: an adoptable, reproducible, releasable product.*
