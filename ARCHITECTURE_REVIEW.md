...
- Getting it running: Docker-only grey-box, regex Dockerfile adaptation that may misdetect my build, pasting JWTs that expire mid-run (worse for `-auth-file` multi-identity: no refresh at all once a token expires).
...
| 22 | ✅ **DONE — Binary constant/string extraction at instrument time** (`instrumentor/Program.cs::ConstantExtractor` — read-only Cecil pass over `Ldstr`/`Ldc_I4`/`Ldc_I4_S`/`Ldc_I8`, deliberately run *before* SharpFuzz's own coverage rewrite so its injected per-branch-site IDs never pollute the pool — confirmed empirically, see the Inconsistencies note below; `.upsidefuzz_constants.jsonl` next to the DLL; `GET /shm/constants` in both hook and source coverage runtimes; `void/go/constants.go` fetches once at startup into `mutateStringCategorized`/`mutateInt`) | P1 | Low | 3–5 d | High | Med | — | Cheap (one existing Cecil pass); free domain dictionary, like `afl -x`. Live-verified against a real Docker build of `fixtures/planted-bug-api/`: `/shm/constants` served real harvested literals (`"CreateItemRequest"`, `"/items/"`, ...) with zero SharpFuzz coverage-instrumentation noise |
| 23 | ✅ **DONE — Response-schema conformance oracle** (`grammarc/oas.py::Operation.response_schemas` — every declared 2xx status, not just the first; `emit_templates.py` flattens to `templates.export.json`'s `response_schemas`; `void/go/schema_oracle.go::checkSchemaConformance`, gated by `-schema-conformance`) — validates live 2xx response bodies against the declared OpenAPI response schema; mismatch = bug | P1 | Low | 3–5 d | — | Med | — | `grammarc/` already parses the spec; a whole free bug class (leaked fields, type drift) was previously ignored. Two distinct, separately-tagged finding classes per explicit user request: `schema_undeclared_field`/`schema_undeclared_sensitive_field` (a field present in a live response the spec never declared — the priority case, since it means a client can read/interact with something undocumented) and the lower-confidence `schema_type_mismatch`. Conservative by design: silent on any endpoint whose grammar declares no response schema at all (no ground truth to compare against), and JSON `null` is always accepted regardless of declared type (avoids a predictable false-positive flood) |
...
**Suggested near-term order (mixing old + new):** trust first — #7 (done) → #25 global seed (done, partial — request-journal replay still open) → **#31 readiness/seeding** → finish #4; then depth — #21 CmpLog (done) → #22 constants (done) → **#5 OAST** ; then new finding classes — #23 schema-conformance (done) → **#26 PII** → **#24 JWT** → **#27 taint**. The cheapest single credibility win remaining is finishing **#25** (a real replay tool, not just a seed).

**Status roll-up for this tranche:** DONE — #21 (CmpLog), #22 (constant extraction), #23 (schema conformance), #25 (partial — global seed only). OPEN — #24, #26, #27, #28, #29, #30, #31, #32.
...
### vs. Schemathesis
- **Property-based generation from JSON Schema (Hypothesis strategies)** with shrinking and stateful links (OpenAPI `links`). UpsideFuzz has no shrinking and no generation-time property engine (its schema conformance oracle, below, is a *response-side* check, not a generation strategy).
- **`--checks` for status-code conformance, content-type conformance.** UpsideFuzz still has neither of these. It does now have a response-*body* conformance oracle (Top-20+ #23, `void/go/schema_oracle.go`) — undeclared fields and declared-vs-observed type drift against the OpenAPI response schema — but that's narrower than Schemathesis's full `--checks` suite.
...
5. ~~**README "instruments arbitrary .NET APIs … almost zero configuration"** vs. reality: Docker required, regex build detection, per-target quickstart runbooks, manual auth token files, RESTler in the loop. Aspirational, not current.~~ **RE-CHECKED 2026-07-25 — mostly stale itself.** The quoted phrasing isn't in the current README (rewritten since this note was written); "RESTler in the loop" is now flatly false (RESTler retired, Top-20 #9/#10 — README says "no RESTler" in three places). "Docker required" isn't a documentation inaccuracy — the tool is Docker-native by design (instrumentation, SHM coverage, and multi-service bring-up all assume it) and nothing claims otherwise. The one part still genuinely true: **auth tokens are manual**, and this was under-documented — README's Quick Start showed only `export AUTH_TOKEN=...`, with no mention that (a) single-identity mode has a built-in login-flow fallback (`AUTH_URL`/`AUTH_BODY`/`AUTH_TOKEN_FIELD`, documented in INSTRUCTIONS.md but not linked from README), or (b) `-auth-file` multi-identity tokens are **never refreshed** once expired — a real gap, since `-auth-file` is the recommended mode for exactly the long access-control-testing runs where tokens are most likely to expire mid-run. **Fixed**: README now states this explicitly next to the `-auth-file` example. The actual capability gap (auto OAuth2/OIDC login *per identity*) is unchanged and tracked separately as #11/Top-20 #13 below — not a doc-drift issue, a real missing feature.
6. ~~**Namespace allowlist substring match** (`fullName.Contains(ns)` in `instrumentor/Program.cs`) — documented Bitwarden footgun; the default is now `--instrument-all-user-code`, so the allowlist path is legacy risk that should be removed or fixed to prefix/exact matching.~~ **✅ FIXED 2026-07-25:** replaced with `MatchesNamespace(fullName, ns)` (exact match or `.`-boundary prefix match) in both `instrumentor/Program.cs` and its `fuzz-prep-multi.py::_FALLBACK_INSTRUMENTOR_CS` mirror. Verified live: a compiled fixture with `Bit.Core.RealCoreType` / `Bit.CoreUtilities.UnrelatedType` (the latter deliberately chosen because `"Bit.CoreUtilities"` starts with the substring `"Bit.Core"`) confirmed only the true `Bit.Core` type is instrumented under `--namespaces Bit.Core` post-fix, where the old `Contains` check would have wrongly matched both. Still legacy-risk in the sense that `--instrument-all-user-code` remains the preferred default; this only hardens the fallback path rather than removing it.
...
12. **`analyzer/RoslynUtil.IsTestPath` still missed a real test-directory shape after item 8's fix: bare "ends with tests" (e.g. `UnitTests/`).** Found 2026-07-25 while writing `analyzer.Tests` (this project's first automated test coverage for `analyzer/`, previously zero). The function's own doc comment already claimed `.../UnitTests/Bar.cs` should match, but the code only checked the dotted plural form (`.tests`, e.g. `MyApp.Tests`) and the bare exact match (`tests`) — never a plain `EndsWith("tests")`. `UnitTests` is arguably the single most common .NET test-directory name, more common than bare `Tests`, so this wasn't a corner case: any target using that convention had its test directory silently parsed as business logic. **Fixed** by adding `segment.EndsWith("tests")` alongside the existing `EndsWith("test")`, accepting the same small false-positive tradeoff already accepted for the singular form (an unusual business directory literally named `Contests` would also now match) — a locked-in, disclosed tradeoff, not an oversight. Regression test: `analyzer.Tests/RoslynUtilTests.cs::IsTestPath_MatchesWholeSegmentsOnly`.
13. **`grammarc/oas.py`'s `OASParser.parse()` never handled Swagger 2.0's `in: body` parameter convention at all.** Found 2026-07-25 while writing `grammarc/test_oas.py`. Only OpenAPI 3.x's `requestBody.content["application/json"].schema` shape populated `operation.request_schema`; a v2 spec's `{"name": "body", "in": "body", "schema": {...}}` parameter — the *standard*, and for most Swashbuckle/NSwag-on-.NET-Framework-era APIs the *only*, way Swagger 2.0 declares a request body — fell through to the `else` branch and was silently misclassified as a query parameter. `request_schema` stayed unset, so `serialize_body` never ran and no JSON body was ever emitted for the operation. `OASParser.__init__` already detects and flags `is_v2`, so this wasn't an unsupported-format gap the class disclaimed — v2 support was assumed to exist and silently didn't, for the specific shape (body params) most v2 specs actually use. **Fixed**: a new `elif hint.location == "body":` branch resolves the raw parameter's `schema` via the same `resolve_schema`/`ref_name` machinery the v3 path already uses. Regression test: `grammarc/test_oas.py::ParseOperationsTests::test_swagger2_body_parameter_becomes_request_schema`.
