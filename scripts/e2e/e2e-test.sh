...
# regression safety net for tools/grammar/grammarc/ + tools/dotnet/analyzer/ (Top-20 #9/#10) and the
...
rm -rf "$PREP_DIR"
python3 "$ROOT_DIR/bin/fuzz-prep-multi.py" --src "$ROOT_DIR/fixtures/planted-bug-api" --out "$PREP_DIR" --main PlantedBugApi \
...
if ! BASE_URL="$BASE_URL" PROBE="/items" "$ROOT_DIR/bin/verify-hook.sh"; then
...
"$ROOT_DIR/bin/compile-grammar.sh" /tmp/e2e-planted-bug-swagger.json --out "$GRAMMAR_DIR" \
...
mkdir -p "$RUN_DIR"
( cd "$ROOT_DIR/src/void" && go build -o /tmp/e2e-smartfuzzergo ./cmd/void ) \
