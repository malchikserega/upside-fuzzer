...
show_help() {
    cat <<'EOF'
run-smart-fuzzer.sh — Launch the SmartFuzzer-Go binary in host mode.
...
Fuzzer binary location:
  smart_fuzzer/go/smartfuzzergo-darwin-arm64  (macOS Apple Silicon)
  smart_fuzzer/go/smartfuzzergo               (Linux amd64, used in Docker)

For full flag reference:
  smart_fuzzer/go/smartfuzzergo-darwin-arm64 --help
  See: smart_fuzzer/README.md
EOF
}
...
FUZZER_BIN=""
for candidate in \
    "$SCRIPT_DIR/smart_fuzzer/go/smartfuzzergo-darwin-arm64" \
    "$SCRIPT_DIR/smart_fuzzer/go/smartfuzzergo-linux-amd64" \
    "$SCRIPT_DIR/smart_fuzzer/go/smartfuzzergo"; do
    if [ -x "$candidate" ]; then
...
if [ -z "$FUZZER_BIN" ]; then
    echo "❌ No fuzzer binary found in smart_fuzzer/go/"
    echo "   Build it first: cd smart_fuzzer/go && go build -o smartfuzzergo ."
    exit 1
fi
...
mkdir -p "$SCRIPT_DIR/smart_fuzzer/crashes" "$SCRIPT_DIR/smart_fuzzer/summaries"

exec "$FUZZER_BIN" -grammar "$SCRIPT_DIR/smart_fuzzer" "$@"
