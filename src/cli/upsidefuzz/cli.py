#!/usr/bin/env python3
"""
cli.py - Single CLI orchestrator for the UpsideFuzz pipeline (Top-20 #19).

Moved here from the repo-root upsidefuzz.py (now a thin compatibility
wrapper, itself relocated to bin/compatibility/upsidefuzz.py) during the
repo-architecture refactor, then again to src/cli/upsidefuzz/cli.py during
the self-contained-module refactor (2026-07-31) -- content and behavior
unchanged both times except ROOT_DIR's climb depth.
...
compile-grammar.sh, verify-hook.sh, or the void engine is modified or
bypassed -- the manual, script-by-script workflow documented in
docs/INSTRUCTIONS.md/QUICKSTART_*.md keeps working exactly as before, this is
purely additive.
...
Two ways to run it:
  ...
  2. Zero-install, via Docker: ./upsidefuzz <subcommand> ...
     ...
     upsidefuzz-cli Docker image (see deployments/docker/Dockerfile.cli),
     which bundles Python, the .NET SDK, and a prebuilt void binary -- so the
     only thing you need installed locally is Docker itself. See docs/CLI.md.
...
# src/cli/upsidefuzz/cli.py -> src/cli/upsidefuzz -> src/cli -> src -> repo root.
...
def _containerize_url(url: Optional[str]) -> Optional[str]:
    """When upsidefuzz.py itself runs inside the CLI Docker image
    (deployments/docker/Dockerfile.cli, launched via the ./upsidefuzz wrapper),
    it and the target's instrumented containers are Docker *siblings*, not
    parent/child -- `localhost`/`127.0.0.1` inside this container refers to
    this container's own loopback, not the host's, so it can never reach a
    port the target publishes on the host. `host.docker.internal` is the
    portable fix (built into Docker Desktop; the launcher adds
...
def _find_void_binary(explicit: Optional[str]) -> str:
    if explicit:
        return explicit
    native = ROOT_DIR / "src" / "void" / "cmd" / "void" / "void"
    ...
    print(
        ...
        "  - Native run: build it with `go -C src/void build -o cmd/void/void ./cmd/void` (needs Go 1.22+), or\n"
        "  - Zero-install: use the Docker launcher `./upsidefuzz fuzz ...` instead, which bundles a prebuilt binary.\n"
        "See src/void/cmd/void/README.md 'Build for Any Platform' for details.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def cmd_instrument(args: argparse.Namespace) -> None:
    ...
    cmd = [
        sys.executable, str(ROOT_DIR / "bin" / "fuzz-prep-multi.py"),
...
def cmd_verify(args: argparse.Namespace) -> None:
    ...
    cmd = [str(ROOT_DIR / "bin" / "verify-hook.sh")]
    if args.up:
        cmd.append("--up")
    if args.down:
        cmd.append("--down")
...
def cmd_grammar(args: argparse.Namespace) -> None:
    ...
    cmd = [str(ROOT_DIR / "bin" / "compile-grammar.sh"), str(swagger_path)]
    if args.dict:
        cmd += ["--dict", args.dict]
    if args.src:
        cmd += ["--src", args.src]
...
def cmd_doctor(_args: argparse.Namespace) -> None:
    ...
    void_bin = ROOT_DIR / "src" / "void" / "cmd" / "void" / "void"
    if void_bin.is_file():
        print(f"  [ok] void binary   found at {void_bin}")
    elif shutil.which("void"):
        print(f"  [ok] void binary   found on PATH ({shutil.which('void')})")
    else:
        print("  [  ] void binary   not built yet -- `go -C src/void build -o cmd/void/void ./cmd/void`, or use ./upsidefuzz (Docker)")
    ...
    if missing_required:
        ...
        print("(builds/uses the upsidefuzz-cli image, see deployments/docker/Dockerfile.cli + docs/CLI.md)")
    else:
