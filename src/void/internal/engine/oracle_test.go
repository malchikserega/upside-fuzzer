...
func TestApplyProfileGatesIndividualOracles(t *testing.T) {
	cfg := config.Config{Profile: "fast", ProbeBOLA: true, ProbeAuthBypass: true, ProbeMassAssign: true}
	config.ApplyProfile(&cfg, map[string]bool{})
	...
	cfg2 := config.Config{Profile: "fast", ProbeBOLA: true}
	config.ApplyProfile(&cfg2, map[string]bool{"probe-bola": true})
	if !cfg2.ProbeBOLA {
...
func TestApplyProfileRespectsExplicitFlags(t *testing.T) {
	...
	cfg := config.Config{Profile: "security"}
	config.ApplyProfile(&cfg, map[string]bool{})
	if !cfg.AccessProbe {
		t.Errorf("security profile should enable access-probe")
	}
	// ...but an explicitly-set flag wins over the profile.
	cfg2 := config.Config{Profile: "security", AccessProbe: false}
	config.ApplyProfile(&cfg2, map[string]bool{"access-probe": true})
	if cfg2.AccessProbe {
		t.Errorf("explicit -access-probe=false must override the security profile")
	}
	// fast profile disables the oracles.
	cfg3 := config.Config{Profile: "fast", InjectionOracle: true, AccessProbe: true}
	config.ApplyProfile(&cfg3, map[string]bool{})
...
// ---------------------------------------------------------------------------
// Valid-chain-gated mass assignment (Phase 3 #117)
// ---------------------------------------------------------------------------

func newMassAssignTestFuzzer() *Fuzzer {
	return &Fuzzer{
		cfg: config.Config{
			AccessProbe:               true,
...
func TestMaybeEnqueueMassAssignProbe_SuppressedWithoutConfirmedResource(t *testing.T) {
	f := newMassAssignTestFuzzer()
	f.activeIDs = []int{1}
	f.meta[1] = TemplateMeta{Method: "PUT", Norm: "/widgets/{param}"}
	// No resource-graph instance for widget "1" was ever recorded -- this
	// write's own path context is unconfirmed.
	res := SendResult{
		Item:   WorkItem{Method: "PUT", Path: "/widgets/1", Identity: "alice", Body: `{"name":"gadget"}`},
		Status: 200, Body: `{"id":"1","name":"gadget"}`,
	}
	f.maybeEnqueueMassAssignProbe(res)
	if len(f.oracleQueue) != 0 {
		t.Fatalf("expected the probe to be suppressed for an unconfirmed resource, got %d enqueued", len(f.oracleQueue))
	}
}

func TestMaybeEnqueueMassAssignProbe_FiresWithConfirmedResource(t *testing.T) {
	f := newMassAssignTestFuzzer()
	f.activeIDs = []int{1}
	f.meta[1] = TemplateMeta{Method: "PUT", Norm: "/widgets/{param}"}
	...
	if len(f.oracleQueue) != 1 {
		t.Fatalf("expected the probe to fire for a confirmed resource, got %d enqueued", len(f.oracleQueue))
	}
}

func TestMaybeEnqueueMassAssignProbe_NestedCreateGatedOnConfirmedParent(t *testing.T) {
	f := newMassAssignTestFuzzer()
	f.activeIDs = []int{1}
	f.meta[1] = TemplateMeta{Method: "POST", Norm: "/organizations/{param}/projects"}
	res := SendResult{
		Item:   WorkItem{Method: "POST", Path: "/organizations/org-7/projects", Identity: "alice", Body: `{"name":"demo"}`},
		Status: 201, Body: `{"id":"1","name":"demo"}`,
	}
	// org-7 was never confirmed -> suppressed.
	f.maybeEnqueueMassAssignProbe(res)
	if len(f.oracleQueue) != 0 {
		t.Fatalf("expected a nested create under an unconfirmed parent to be suppressed, got %d enqueued", len(f.oracleQueue))
	}

	// Now confirm org-7 exists -> fires.
	f.resourceGraph.recordInstance(
		ResourceIdentity{ResourceType: "organization", IdentityKind: "scalar", NormalizedValue: normalizeResourceValue("organization", "scalar", "org-7"), RawValue: "org-7"},
		"POST /organizations", "seq-1", LifecycleCreated, 0.9,
	)
	f.maybeEnqueueMassAssignProbe(res)
	if len(f.oracleQueue) != 1 {
		t.Fatalf("expected the probe to fire once the parent organization is confirmed, got %d enqueued", len(f.oracleQueue))
	}
}

func TestMaybeEnqueueMassAssignProbe_FlatPathPassesThroughUnconditionally(t *testing.T) {
	f := newMassAssignTestFuzzer()
	f.activeIDs = []int{1}
	f.meta[1] = TemplateMeta{Method: "POST", Norm: "/widgets"} // no path params -- nothing to confirm
	res := SendResult{
		Item:   WorkItem{Method: "POST", Path: "/widgets", Identity: "alice", Body: `{"name":"gadget"}`},
		Status: 201, Body: `{"id":"1","name":"gadget"}`,
	}
	f.maybeEnqueueMassAssignProbe(res)
	if len(f.oracleQueue) != 1 {
		t.Fatalf("expected a flat top-level create (no parent/self context to confirm) to fire unconditionally, got %d enqueued", len(f.oracleQueue))
	}
}

func TestMaybeEnqueueMassAssignProbe_ResourceGraphDisabledBypassesGate(t *testing.T) {
	f := newMassAssignTestFuzzer()
	...
	f.maybeEnqueueMassAssignProbe(res)
	if len(f.oracleQueue) != 1 {
		t.Fatalf("expected -resource-graph=false to bypass the confirmation gate entirely, got %d enqueued", len(f.oracleQueue))
	}
}
...
func TestApplyProfileGatesProbeDifferential(t *testing.T) {
	cfg := config.Config{Profile: "fast", ProbeDifferential: true}
	config.ApplyProfile(&cfg, map[string]bool{})
	if cfg.ProbeDifferential {
		t.Errorf("fast profile should disable probe-differential")
	}

	cfg2 := config.Config{Profile: "fast", ProbeDifferential: true}
	config.ApplyProfile(&cfg2, map[string]bool{"probe-differential": true})
