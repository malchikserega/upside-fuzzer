...
type Fuzzer struct {
	...
	activeIDs []int
	// templatesPath/templatesHash (manifest.go, Phase 5 #127) record exactly
	// which exported templates JSON this run actually loaded and its content
	// hash, for the run manifest -- the one piece of grammar/spec provenance
	// void can verify itself, as opposed to TargetImageDigest/OpenAPISpecHash
	// (Config), which are unverified pass-through labels from the caller.
	...
	// Race-burst outcome tracking (race.go, Phase 4 #119): burstID -> running
	// success count / results-seen count, evaluated once every member of a
	// burst has reported in; raceBurstEvaluated dedups so a burst is scored
	// exactly once even if a stray extra result arrives.
	...
	// maxChainDepthSeen is this run's own "personal best" sequence chain
	// depth reached so far -- feeds maxChainDepthBonus (sequence.go), the
	// composite reward's "valid_chain_depth" term
	// (docs/ARCHITECTURE_STATEFUL.md §2.6).
	maxChainDepthSeen int
	// lastCheckpointSave gates maybeSaveCheckpoint's periodic cadence
	// (checkpoint.go) -- zero-value Time means "never saved yet," so the
	// first eligible tick always saves regardless of CheckpointIntervalSec.
	...
	accessFindings   int
	// Adversarial branch planner state (adversarial.go, docs/ARCHITECTURE_STATEFUL.md
	// §2.4): stale-object/stale-ETag/workflow-bypass findings, each taking an
	// already-confirmed-real resource-graph instance as their starting point
	// rather than replaying the current request under a different identity.
	adversarialProbeCount map[string]int
	adversarialSeen       map[string]struct{}
	adversarialFindings   int
	...
	// producerConsumerBindings is a bounded, explicit log of "this follow-up
	// request's field was bound to this exact value, which came from this
	// exact prior response field" events (ProducerConsumerBinding,
	// sequence.go) -- same single-main-loop-goroutine ownership as
	// resourceGraph, same reasoning, no mutex needed.
	producerConsumerBindings []ProducerConsumerBinding
	// bindingOrder is producerConsumerBindings' own monotonic logical clock,
	// deterministic under -seed (not wall time) -- mirrors ResourceGraph.order.
	bindingOrder uint64
}

func NewFuzzer(cfg config.Config) (*Fuzzer, error) {
	...
	templatesHash := sha256FileHash(templatesPath)
...
func shouldRefreshTemplates(cfg config.Config, templatesPath string) bool {
...
func (f *Fuzzer) Run() error {
	...
	resumed := false
	if f.cfg.Resume {
		loaded, err := f.LoadCheckpoint(f.cfg.CheckpointPath)
		if err != nil {
			fmt.Printf("Checkpoint resume warning: %v (starting fresh)\n", err)
		} else if loaded {
			resumed = true
			fmt.Printf("Resumed from checkpoint: %s (corpus=%d resource_instances=%d)\n",
				f.cfg.CheckpointPath, len(f.corpus), f.resourceGraph.instanceCount())
		}
	}
	if !resumed {
	...
	err = f.mainLoop()
	if saveErr := f.SaveCheckpoint(f.cfg.CheckpointPath); saveErr != nil && f.cfg.CheckpointPath != "" {
...
func (f *Fuzzer) buildTemplateMetaAndDependencies() {
	...
	for _, t := range f.templates {
		...
		f.meta[t.ID] = TemplateMeta{
			Method: item.Method, Path: item.Path, Norm: norm, ContentType: ct,
			XStateTransition: t.XStateTransition, RoslynAction: t.RoslynAction,
		}
