...
// sarifChainProperty renders a finding's sequence chain (CrashFinding.ChainTrace,
// Phase 5 #127) as a SARIF result property -- nil (omitted from the marshaled
// JSON as "null") for the common case of a non-sequence finding, so a viewer's
// "chain" column reads as genuinely absent rather than an empty array.
func sarifChainProperty(fd CrashFinding) any {
	if len(fd.ChainTrace) == 0 {
		return nil
	}
	steps := make([]map[string]any, 0, len(fd.ChainTrace))
	for _, s := range fd.ChainTrace {
	...
	bindings := make([]map[string]any, 0, len(fd.Bindings))
	for _, b := range fd.Bindings {
		bindings = append(bindings, map[string]any{
			"producer_op":   b.ProducerOp,
			"consumer_op":   b.ConsumerOp,
			"resource_type": b.ResourceType,
			"value":         b.Value,
		})
	}
	return map[string]any{
		"sequence_id": fd.SequenceID,
		"depth":       len(steps),
		"steps":       steps,
		"bindings":    bindings,
	}
		...
		"runs": []map[string]any{
			{
				...
				"results": results,
				// properties.run_manifest (Phase 5 #127): the same reproducibility
				// manifest report.go's JSON output carries, at SARIF's own
				// documented per-run extension point -- image digest, grammar/
				// templates hash, seed, target -- so a SARIF-only consumer (GitHub
				// code scanning, DefectDojo, ...) doesn't lose that provenance.
				"properties": map[string]any{
					"run_manifest": f.buildRunManifest(),
				},
			},
		},
	}
