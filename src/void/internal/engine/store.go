...
type RuntimeStore struct {
	...
	// pathValues (body_bind.go, typed structural mutation Stage 6) is a
	// path-qualified sibling of `values`, keyed "resourceType.dotted.path"
	// instead of a bare canonical field name -- so two different nested "id"
	// fields under different parents (e.g. order.customer.id vs.
	// order.shipping.id) no longer collide in one shared bucket the way the
	// flat `values` map always has. Populated alongside (not instead of) the
	// existing flat write in extractJSONRuntimeValues (sequence.go); the flat
	// map remains the coarser fallback it already was for every existing
	// caller.
...
// recordPathValue/getPathValues: see the pathValues field doc comment above.
// scopedPath is caller-constructed (resourceType + "." + dotted schema path);
// this file makes no assumption about its exact shape beyond "a stable,
// unique string per schema position."
func (r *RuntimeStore) recordPathValue(scopedPath, value string) {
	v := normalizeValue(value)
	if !isUsefulValue(v) || strings.ContainsAny(v, "{}$") || scopedPath == "" {
		return
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	const maxPerPath = 16
	existing := r.pathValues[scopedPath]
	...
	if len(existing) > maxPerPath {
		existing = existing[len(existing)-maxPerPath:]
	}
	r.pathValues[scopedPath] = existing
}

func (r *RuntimeStore) getPathValues(scopedPath string) []string {
	r.mu.RLock()
	defer r.mu.RUnlock()
	vals := r.pathValues[scopedPath]
...
//
// Scope note: unlike pickFollowupPathValue (sequence.go), this body/query-field
// bias is NOT tenant-scoped -- it has no SequenceState in hand at this call
// depth (template.go's rendering chain doesn't thread one through), so a
// biased body/query field can still pull in a same-typed instance from a
// different tenant. Path-placeholder substitution (the primary, most-used
// binding mechanism) is the one this pass scoped; extending tenant-awareness
// down through the payload-biasing call chain is tracked as follow-up work,
