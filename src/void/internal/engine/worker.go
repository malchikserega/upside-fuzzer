...
func (f *Fuzzer) handleResult(res SendResult) {
	// Race-burst outcome tracking (race.go) counts EVERY member's result,
	// including a network-error one -- otherwise a burst with even one
	// errored-out member would never reach BurstSize and would just leak
	// until the defensive eviction cap kicks in, silently losing whatever
	// signal the OTHER members of that same burst carried.
	if res.Item.BurstID != "" {
		f.recordRaceBurstResult(res)
	}
	if res.Err != nil {
		f.totalErrors++
		return
	...
	patchTraceStatus(res.Item, res.Status)
	...
	recordStructuralCategoryAttempt(res.Item.MutationLabel)
	if edgeShare > 0 {
		ms.NewEdges += edgeShare
		recordMutationCategoryHit(res.Item.MutationLabel)
		recordStructuralCategoryHit(res.Item.MutationLabel)
	...
	if res.Status >= 200 && res.Status < 300 {
		...
		// Adversarial branch planner (adversarial.go): stale-ETag replay against a
		// resource with a known prior version -- does this write correctly get
		// rejected as a conflicting/out-of-date update?
		f.maybeEnqueueStaleETagProbe(res)
		// Idempotency-replay (idempotency.go): verbatim replay of a just-succeeded
		// create-shaped POST -- does it correctly avoid creating a duplicate?
