...
func (f *Fuzzer) extractResourceCandidates(body string, headers map[string]string, sourceOp string) []ExtractedCandidate {
	...
	for _, h := range extractHeaderCandidates(headers) {
		...
		// Query parameters are orthogonal to path segments -- an async
		// operation's poll/status URI (Location, Link) often carries its
		// operation id as a query param (e.g. "?jobId=..."), not a path
		// segment, regardless of whether the path itself matched a template.
		out = append(out, extractQueryParamCandidates(h.URI, sourceOp)...)
	}

	// Set-Cookie doesn't require a JSON body (a bare 204/302 can still carry a
	// resource-identifying cookie), so this runs before the JSON-body early
	// return below.
	out = append(out, extractCookieCandidates(headers, sourceOp)...)
	...
	for _, hl := range extractHALCandidates(js, sourceOp, 0) {
		...
		out = append(out, extractQueryParamCandidates(hl.URI, sourceOp)...)
	}
...
// confQueryParamWithNameHint/confQueryParamBare/confCookieWithNameHint/
// confCookieBare mirror the structural-extraction confidence tiers
// (confShapeWithNameHint/confShapeUUIDBare etc.) for the two extraction
// sources below -- a shape match corroborated by the param/cookie name
// scores higher than a bare shape match with no name signal.
const (
	confQueryParamWithNameHint = 0.65
	confCookieWithNameHint     = 0.60
)

// extractQueryParamCandidates finds resource-shaped values in rawURI's own
// query string -- a source the structural/HAL/JSON:API/header extraction
// above never looks at, since matchRouteTemplateCandidates only ever
// considers a URI's PATH segments. This is what makes an async operation's
// poll/status URI (e.g. "/operations/status?jobId=abc-123", or a request's
// own "?operationId=..." query) extractable at all: the operation id often
// lives in a query parameter, not a path segment. Deterministic (sorted key
// order), since url.Values iteration order is randomized otherwise and this
// engine cares about determinism under a fixed -seed.
func extractQueryParamCandidates(rawURI, sourceOp string) []ExtractedCandidate {
	u, err := url.Parse(strings.TrimSpace(rawURI))
	...
	for _, key := range keys {
		...
		v := vals[0]
		kind, base, shaped := classifyValueShape(v)
		if !shaped {
			continue
		}
		rt := resourceTypeFromKeyName(key)
		conf := base
		if rt != "" {
			conf = confQueryParamWithNameHint
		} else {
			rt = resourceTypeFromSourceOp(sourceOp)
		}
		out = append(out, ExtractedCandidate{
			RawValue: v, NormalizedValue: v, ValueType: identityKindOf(kind),
			ResourceType: rt, SourceOperation: sourceOp,
			JSONPath: "?" + key, Strategy: "query_param", Confidence: conf,
		})
	}
	return out
}

// extractCookieCandidates finds a resource-shaped value in a response's
// Set-Cookie header -- another source the rest of the pipeline never
// consults. Only the cookie's own name=value pair is used (everything after
// the first ";" is attributes: Path, Expires, HttpOnly, SameSite, ...), and
// only the FIRST Set-Cookie header is available at all -- worker.go's own
// response-header population keeps only vals[0] per header name, so a
// response setting multiple cookies only ever exposes one here. That's an
// existing, separate limitation of how SendResult.Headers is populated, not
// something this function can work around.
func extractCookieCandidates(headers map[string]string, sourceOp string) []ExtractedCandidate {
	...
	kind, base, shaped := classifyValueShape(value)
	if !shaped {
		return nil
	}
	rt := resourceTypeFromKeyName(name)
	conf := base
	if rt != "" {
		conf = confCookieWithNameHint
	} else {
		rt = resourceTypeFromSourceOp(sourceOp)
	}
	return []ExtractedCandidate{{
		RawValue: value, NormalizedValue: value, ValueType: identityKindOf(kind),
		ResourceType: rt, SourceOperation: sourceOp,
		HeaderName: "Set-Cookie", JSONPath: name, Strategy: "cookie", Confidence: conf,
	}}
}

// extractVersionHeader returns a response's ETag header value
// (case-insensitively), used as ResourceInstance.Version -- the
// optimistic-locking signal a stale-ETag oracle needs. Deliberately narrow
// (ETag only, not "any header that might be a version") since ETag is the one
// HTTP-standard, unambiguous version signal; anything else would be
// target-specific guessing. Checks "Etag" first -- worker.go populates
// SendResult.Headers directly from Go's net/http.Header, whose keys are
// always MIME-canonicalized ("ETag" -> "Etag", not "ETag" or "etag") -- then
// falls back to the literal and all-lowercase spellings for any caller that
// hands in a non-canonicalized map (as this package's own tests sometimes do).
func extractVersionHeader(headers map[string]string) string {
	for _, name := range [...]string{"Etag", "ETag", "etag"} {
...
// maxAttributeFields/maxAttributeValueLen bound extractTopLevelAttributes'
// output the same way LatestRepr/Aliases/ChildKeys are already bounded
// elsewhere in this package -- a resource's Attributes are a small oracle-input
// snapshot (e.g. {"status": "draft"}), not a full body mirror.
const (
	maxAttributeFields   = 8
	maxAttributeValueLen = 200
)

// extractTopLevelAttributes returns a small, bounded, deterministically-chosen
// (sorted key order, so it doesn't depend on Go's randomized map iteration)
// snapshot of a response body's top-level scalar fields, for
// ResourceInstance.Attributes. Nested objects/arrays are skipped -- this is a
// shallow state-flag snapshot, not a generalized body summarizer.
func extractTopLevelAttributes(body string) map[string]any {
	if !reJSONStartAny.MatchString(body) {
		return nil
	}
	var js any
	...
	out := make(map[string]any, maxAttributeFields)
	for _, k := range keys {
		if len(out) >= maxAttributeFields {
			break
		}
		switch tv := m[k].(type) {
		case string:
			if len(tv) > maxAttributeValueLen {
				tv = tv[:maxAttributeValueLen]
			}
			out[k] = tv
		case bool, float64:
			out[k] = tv
		default:
			continue // skip nested objects/arrays/null
		}
	}
	if len(out) == 0 {
		return nil
...
//
// An action-verb-shaped trailing segment (approve/refund/download/process/...,
// knownTransitionActionVerbs, resource_scheduling.go) names the OPERATION,
// not the resource -- found via TestScoreConsumer_DownloadBonusForMultipartSourcedFile
// (Phase 4 #122) returning "download" instead of "file" for
// /files/{id}/download, which silently broke every resource-graph lookup
// keyed off an action-suffixed endpoint's own inferred type (availability
// scoring, download/poll bonuses, ...). When the last segment is a known
// verb and a preceding segment exists, that preceding segment is preferred.
func resourceTypeFromEndpointPath(path string) string {
	...
	if len(tokens) >= 2 {
		if _, isVerb := knownTransitionActionVerbs[last]; isVerb {
			return singularize(tokens[len(tokens)-2])
		}
	}
