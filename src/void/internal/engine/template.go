...
func (f *Fuzzer) renderTemplateContext(templateID int, mutateMode string, havocDepth int, seedIdx int, ctx *WorkItem) (WorkItem, error) {
	...
	// Typed structural body mutation (requirement #2/#3): when the grammar
	// declared a body_schema for this template, build/mutate a typed value
	// tree instead of the legacy flat-segment body above -- Body stays a
	// throwaway placeholder for a typed item until prepareItemForSend
	// serializes BodyTree into it immediately before send. Templates with no
	// BodySchema (every grammar predating this feature) take neither branch
	// here and fall straight through to the unmodified legacy mutateJSONBody
	// path below, exactly as before this feature existed.
	var bodyTree *BodyValue
	if f.cfg.TypedBodyMutation && t.BodySchema != nil {
		mode := "valid"
		if (mutateMode == "mutate" || mutateMode == "havoc") && rand.Float64() < f.cfg.AdversarialBodyRate {
			mode = "adversarial"
		}
		var opLabel string
		bodyTree, opLabel = f.buildAndMutateBodyTree(t.BodySchema, mode, ctx)
		if opLabel != "" {
			mutParts = append(mutParts, opLabel)
		}
	}

	if bodyTree == nil && (mutateMode == "mutate" || mutateMode == "havoc") && (method == "POST" || method == "PUT" || method == "PATCH") {
		chance := 0.15
		if mutateMode == "havoc" {
			chance = 0.30
		}
...
func (f *Fuzzer) prepareItemForSend(item WorkItem) WorkItem {
	// Typed structural body (requirement #6): structure is preserved as a tree
	// through render/mutate/schedule; this is the one place -- immediately
	// before the HTTP body reader is built (sendOneWithClient calls this as
	// its first line) -- where it becomes wire bytes. item.Body for a typed
	// item is a throwaway placeholder until this runs. Must happen before the
	// AutoAntiForgery early return below, or a typed item would be sent with
	// its stale placeholder Body whenever anti-forgery injection is disabled.
	if item.BodyTree != nil {
		ct := canonicalContentType(item.Headers)
		switch {
		case strings.Contains(ct, "multipart/form-data"):
			boundary := newBodyTreeMultipartBoundary()
			item.Body = item.BodyTree.ToMultipart(boundary)
			setHeaderCI(item.Headers, "Content-Type", "multipart/form-data; boundary="+boundary)
		case strings.Contains(ct, "application/x-www-form-urlencoded"):
			item.Body = item.BodyTree.ToForm()
		default:
			item.Body = item.BodyTree.ToJSON()
		}
	}
