...
// TestRecordCrash_PopulatesChainTraceFromSeqState verifies the finding's
// ChainTrace/SequenceID (Phase 5 #127, report.go/sarif.go consumers) are
// populated from the originating WorkItem's SequenceState.History, and left
// empty for a crash with no sequence chain at all.
func TestRecordCrash_PopulatesChainTraceFromSeqState(t *testing.T) {
	f := newCrashTestFuzzer(t)
	res := SendResult{
		Item: WorkItem{
			Method: "PUT", Path: "/widgets/1",
			MutationLabel: "seq(d1:GET /widgets->PUT)", MutationName: "sequence",
			SeqState: &SequenceState{ID: "seq-chain-1", History: []SequenceStep{
	...
	fd := f.findings[0]
	if fd.SequenceID != "seq-chain-1" {
		t.Errorf("expected SequenceID=seq-chain-1, got %q", fd.SequenceID)
	}
	if len(fd.ChainTrace) != 2 {
		t.Fatalf("expected a 2-step ChainTrace, got %v", fd.ChainTrace)
	}
	if fd.ChainTrace[0].Path != "/widgets" || fd.ChainTrace[1].Path != "/widgets/1" {
		t.Errorf("expected chain trace steps in order, got %v", fd.ChainTrace)
	}
}

func TestRecordCrash_NoSequenceStateLeavesEmptyChainTrace(t *testing.T) {
	f := newCrashTestFuzzer(t)
	res := mkCrashResult("GET", "/items/1", 500, "NullReferenceException", "boom")
	...
	fd := f.findings[0]
	if fd.SequenceID != "" || len(fd.ChainTrace) != 0 {
		t.Errorf("expected empty SequenceID/ChainTrace for a non-sequence crash, got %q / %v", fd.SequenceID, fd.ChainTrace)
	}
}
