...
func (f *Fuzzer) recordCrash(res SendResult) bool {
	...
	// Whole-chain minimization (minimize.go, Phase 5 #126): for a crash
	// reached through a multi-step sequence chain, also try dropping
	// non-essential earlier STEPS entirely -- a distinct concern from the
	// field-level minimization above, which only ever shrinks the final
	// crashing request's own query/form/JSON/path.
	if f.cfg.MinimizeCrash && f.cfg.MinimizeChain && !skipExpensive {
		t0 := time.Now()
		if newHistory, changed, probes := f.minimizeChainCandidate(pocItem); changed {
			shrunkState := *pocItem.SeqState
			shrunkState.History = newHistory
			pocItem.SeqState = &shrunkState
			minimized["chain_changed"] = true
			minimized["chain_probes"] = probes
			minimized["chain_steps"] = len(newHistory)
			rec.Minimized = minimized
		} else if probes > 0 {
			minimized["chain_changed"] = false
			minimized["chain_probes"] = probes
		}
		f.triageTimeSpent += time.Since(t0)
	}
	...
	f.findings = append(f.findings, CrashFinding{
		...
		SequenceID:   seqID,
		ChainTrace:   cloneChainTrace(pocItem.SeqState),
